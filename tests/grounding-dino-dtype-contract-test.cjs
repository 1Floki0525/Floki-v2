...
const worker = fs.readFileSync(
  path.join(root, '.floki-tools/grounding-dino/grounding-dino-worker.py'),
  'utf8'
);

assert.match(worker, /DTYPE\s*=\s*torch\.float32/);
assert.doesNotMatch(worker, /torch\.float16/);
assert.match(worker, /dtype=DTYPE/);
