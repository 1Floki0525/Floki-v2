...
function main() {
  ...
  assert.match(reconciler, /filter\(\(gate\) => gate !== 'client_ready' && gate !== 'window_visible'\)/, 'chat-local-runtime must ignore client visibility gates for host senses');
  ...
  console.log(JSON.stringify({
    ...
    client_visibility_is_telemetry_only: true,
