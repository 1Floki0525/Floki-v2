...
const sleep = loadYamlFile(fixtureFile).sleep;
...
const legacyState = {
  ...
  rem_cycles: [{ cycle_number: 1, scheduled_at: start.toISOString(), status: 'pending' }],
...
assert.deepEqual(
  migrated.rem_cycles.map((cycle) => new Date(cycle.scheduled_at).getTime() - start.getTime()),
  [10 * 60000, 20 * 60000]
);
assert.equal(claimDueRemCycle({ state_file: stateFile, now: start, sleep_config: sleep }), null);

for (const [cycleNumber, minute] of [[1, 10], [2, 20]]) {
...
assert.equal(completed.rem_cycles.filter((cycle) => cycle.status === 'complete').length, 2);
...
assert.equal(fresh.first_rem_offset_minutes, 10);
assert.equal(fresh.rem_cycles.length, 2);
assert.equal(fresh.rem_cycles[0].scheduled_at, new Date(start.getTime() + 10 * 60000).toISOString());
...
console.log('FLOKI_V22_MANUAL_NAP_TWO_TEN_MINUTE_REM_CONTRACT_PASS');
