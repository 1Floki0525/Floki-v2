...
assert.match(backend, /isLegacyLocalEndpoint/);
...
assert.match(backend, /Unauthorized: missing, expired, or invalid approved-user session credential/);
...
assert.match(viewModel, /sessionCredential: String/);
assert.match(viewModel, /normalizeFlokiSessionCredential\(sessionCredential\)/);
assert.match(viewModel, /Public gateway profiles require an approved-user session credential/);
...
assert.match(ui, /Approved-user session credential/);
assert.match(ui, /Developer local profile/);
assert.match(ui, /CLEAR SESSION/);
