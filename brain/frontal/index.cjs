...
function compactCognitionContext(context = {}) {
  ...
  return Object.freeze({
    ...
    chat_webcam_vision: {
      ...
      uncertain_objects: Array.isArray(webcamVision.uncertain_objects) ? webcamVision.uncertain_objects.slice(0, 16) : [],
...
function buildCognitionPrompt(context) {
  ...
  return [
    ...
    '- uncertain_objects contains transient or weakly-supported detections that have not yet been temporally confirmed; express these only with explicit hedging (\'possibly\', \'something that might be\') and only when directly relevant; do not treat them as facts.',
