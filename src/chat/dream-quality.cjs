...
function assertNovelty(dream, context) {
  ...
  const opening = String(dream.dream_story || '')
    .split(/\s+/)
    .slice(0, 55)
    .join(' ');
  const symbols = new Set((dream.symbols || []).map(normalizeTitle));
  ...
  for (const prior of recent) {
    ...
    if (
      title &&
      priorTitle &&
      (title === priorTitle || overlap(title, priorTitle) >= 0.8)
    ) {
      throw new Error('dream title is too similar to a recent dream: ' + prior.title);
    }
    ...
    if (
      ...
      prior.story_opening &&
      opening &&
      overlap(opening, prior.story_opening) >= 0.76
    ) {
      throw new Error('dream opening is too similar to a recent dream');
    }

    const priorSymbols = new Set(
      (prior && prior.symbols || []).map(normalizeTitle)
    );

    let repeated = 0;
    for (const symbol of symbols) {
      if (symbol && priorSymbols.has(symbol)) repeated += 1;
    }

    if (symbols.size >= 3 && repeated >= Math.ceil(symbols.size * 0.67)) {
      throw new Error('dream reuses too many symbols from a recent dream');
    }
  }
}
...
function buildQualityRegenerationPrompt(error, previousJson, context, options = {}) {
  return [
    ...
    'Regenerate the complete dream once. Do not merely patch or shorten individual fields.',
